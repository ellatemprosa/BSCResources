...
title1 'DCCT nephropathy prevalence at 6 years in the cohort with high baseline AER';
...
proc logistic descending;
  ...
  / rl covb rsquare;
units int =1 -1 hbael=1 yearsdm=1 sbp=1 5
  female=1 -1;
output out=two xbeta=xbeta pred=pred;
title2 'logistic regression model';
 
proc univariate data=two; var pred;
title2 'ssq of pred values for R-square resid calculation';
 
run;
 
proc genmod data = renal;
class int female;
model micro24 = int hbael yearsdm sbp female
  / dist=binomial link=logit type3;
title2 'logistic regression model through Proc Genmod';
run;
